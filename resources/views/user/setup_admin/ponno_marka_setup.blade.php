...
<form action="{{ isset($data) ? route('ponno_marka_setup.update',$data->id) : '' }}" id="form_data" method="POST">
@csrf
@method('PUT')
      ...
      <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6">
        <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
          <div class="text-gray-600 mb-2 text-center">
            <p class="font-medium text-lg">পন্যের মার্কা সেটাপ</p>
          </div>
          ...
          <div class="lg:col-span-2">
            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
              <div class="md:col-span-3">
                <label for="ponno_marka">পন্যের মার্কা :</label>
                <input type="text" name="ponno_marka" id="ponno_marka" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ isset($data) ? $data->ponno_marka : '' }}" required/>
                @if($errors->has('ponno_marka'))
                <span class="text-sm text-red-600">{{ $errors->first('ponno_marka') }} </span>
                @endif
    ...
    function loadCurrentData()
    {
      $(function () {
        ...
        var table = $('.data-table').DataTable({
            ...
            ajax: "{{ route('ponno_marka_setup.admin') }}",
            columns: [
                {data: 'sl', name: 'sl'},
