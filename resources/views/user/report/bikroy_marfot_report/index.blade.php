      ...
      <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6">
        <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
          <div class="text-gray-600 mb-2 text-center">
            <p class="font-medium text-lg">বিক্রয় মারফত রিপোর্ট</p>
          </div>
          ...
          <div class="lg:col-span-2">
            ...
            <div class="md:col-span-1">
                <label for="marfot_id">মারফতের নাম :</label>
                <select name="marfot_id" id="marfot_id" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" required>
                    ...
                    @if($bikroy_marfot_setup)
                    @foreach($bikroy_marfot_setup as $v)
...
<script>
  ...
  function search()
  {
    var marfot_id = $('#marfot_id').val();
    var sales_type = $('#sales_type').val();
    var ponno_setup_id = $('#ponno_setup_id').val();
    var date_from = $('#date_from').val();
    var date_to = $('#date_to').val();
    if(date_from != "" && date_to != "" && marfot_id != "")
    {
      $.ajax({
      type : 'POST',
      url : '{{route("bikroy_marfot_report.search")}}',
      data :  {
        marfot_id : marfot_id,
