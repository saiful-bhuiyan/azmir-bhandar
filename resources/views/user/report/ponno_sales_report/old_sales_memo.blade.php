...
<div class="container mx-auto max-w-screen ">
    <div class="p-4 px-4 mb-6 text-center bg-white md:p-8">
      ...
      <div class="grid grid-cols-4 gap-4 pt-4 text-sm text-left gap-y-2 md:grid-cols-4">
        ...
        <div class="col-span-2">
          <p class="text-sm font-bold text-red-600">বিক্রয় মারফত : {{$sales->bikroy_marfot_setup->marfot_name}}</p>
        </div>
        <div class="col-span-2">
        ...
        <div class="mx-auto">
          <table class="w-full text-sm text-center border border-collapse border-slate-500">
            ...
            <tbody>
               ...
               <tr>
                  <td colspan="7" class="border border-slate-500 font-bold text-right px-2">(+) ক্রেতা কমিশন : </td>
                  <td class="border border-slate-500"> {{$total_kreta_commission}}</td>
               </tr>
               @if($labour > 0)
                <tr>
                  <td colspan="7" class="border border-slate-500 font-bold text-right px-2">(+) লেবার : </td>
                  <td class="border border-slate-500"> {{$labour}}</td>
                </tr>
               @endif
               @if($other > 0)
                <tr>
                  <td colspan="7" class="border border-slate-500 font-bold text-right px-2">(+) অন্যান্য : </td>
                  <td class="border border-slate-500"> {{$other}}</td>
                </tr>
               @endif
               @if($sales->discount > 0)
                <tr>
                  <td colspan="7" class="border border-slate-500 font-bold text-right px-2">(-) ডিসকাউন্ট : </td>
               ...
               @endphp
                <tr>
                    <td colspan="7" class="font-bold border border-slate-500 text-right px-2">মোট টাকা : </td>
                ...
                @endphp
                <tr>
                    <td colspan="7" class="font-bold border border-slate-500 text-right px-2">(+) সাবেক : </td>
                    <td class="font-bold border border-slate-500">{{$sabek - $current_sale}}</td>
                </tr>
                <tr>
                    <td colspan="7" class="font-bold border text-base border-slate-500 text-right px-2">সর্বমোট : </td>
