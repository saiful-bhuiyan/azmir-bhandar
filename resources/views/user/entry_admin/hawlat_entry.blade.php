...
<form action="{{ isset($data) ? route('hawlat_entry.update',$data->id) : '' }}" id="form_data" method="POST">
@csrf
@method('PUT')
      ...
      <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6">
        <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
          <div class="text-gray-600 mb-2 text-center">
            <p class="font-medium text-lg">হাওলাত জমা/খরচ এডমিন</p>
          </div>
          ...
          <div class="lg:col-span-2">
            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
              ...
              <div class="md:col-span-2">
                ...
                <select name="address" id="address" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" onchange="return getHawlatNameByAddress();" required>
                    <option value="" selected>সিলেক্ট</option>
                    @if($hawlat_setup)
                    @foreach($hawlat_setup as $v)
                    <option value="{{$v->address}}" {{ isset($data) && $data->hawlat_setup->address == $v->address ? 'selected' : '' }} >{{$v->address}}</option>
                    @endforeach
                    @endif
                </select>
              ...
              <div class="md:col-span-2">
                ...
                <select name="hawlat_setup_id" id="hawlat_setup_id" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" required>
                  <option value="{{ isset($data) ? $data->hawlat_setup->id : '' }}" selected>{{ isset($data) ? $data->hawlat_setup->name : 'সিলেক্ট' }}</option>
                </select>
