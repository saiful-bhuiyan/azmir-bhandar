...
<form action="{{ isset($data) ? route('kreta_joma_entry.update',$data->id) : '' }}" id="form_data" method="POST">
@csrf
@method('PUT')
      ...
      <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6">
        <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
          <div class="text-gray-600 mb-2 text-center">
            <p class="font-medium text-lg">ক্রেতার জমা এডমিন</p>
          </div>
            ...
            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
              ...
              <div class="md:col-span-1">
                ...
                <select name="area" id="area" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" onchange="return getkretaAddressByArea();" required>
                    ...
                    @if($kreta_setup)
                    @foreach($kreta_setup as $v)
                    <option value="{{$v->area}}" {{ isset($data) && $data->kreta_setup->area == $v->area ? 'selected' : '' }} >{{$v->area}}</option>
                    @endforeach
                    @endif
                </select>
                @if($errors->has('area'))
              ...
              <div class="md:col-span-2">
                ...
                <select name="address" id="address" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" onchange="return getKretaNameByAddress();" required>
                    <option value="{{ isset($data) ? $data->kreta_setup->address : '' }}" selected>{{ isset($data) ? $data->kreta_setup->address : 'সিলেক্ট' }}</option>
                </select>
              ...
              <div class="md:col-span-2">
                ...
                <select name="kreta_setup_id" id="kreta_setup_id" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" required>
                    <option value="{{ isset($data) ? $data->kreta_setup->id : '' }}" selected>{{ isset($data) ? $data->kreta_setup->name : 'সিলেক্ট' }}</option>
                </select>
    ...
    function loadCurrentData()
    {
      $(function () {
        ...
        var table = $('.data-table').DataTable({
            ...
            ajax: "{{ route('kreta_joma_entry.admin') }}",
            columns: [
                {data: 'sl', name: 'sl'},
                {data: 'area', name: 'area'},
