...
<form action="{{ isset($data) ? route('other_joma_khoroc_entry.update',$data->id) : '' }}" id="form_data" method="POST">
@csrf
@method('PUT')
      ...
      <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6">
        <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
          <div class="text-gray-600 mb-2 text-center">
            <p class="font-medium text-lg">অন্যান্য জমা/খরচ এডমিন</p>
          </div>
          ...
          <div class="lg:col-span-2">
            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
              ...
              <div class="md:col-span-2">
                <label for="other_id">খাতের নাম :</label>
                <select name="other_id" id="other_id" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" required>
                    <option value="" selected>সিলেক্ট</option>
                    @if($other_joma_khoroc_setup)
                    @foreach($other_joma_khoroc_setup as $v)
                    <option value="{{$v->id}}" {{ isset($data) && $data->other_joma_khoroc_setup->id == $v->id ? 'selected' : '' }} >{{$v->name}}</option>
                    @endforeach
                    @endif
                </select>
                @if($errors->has('other_id'))
                <span class="text-sm text-red-600">{{ $errors->first('other_id') }} </span>
                @endif
              </div>
    ...
    function loadCurrentData()
    {
      $(function () {
        ...
        var table = $('.data-table').DataTable({
            ...
            ajax: "{{ route('other_joma_khoroc_entry.admin') }}",
            columns: [
                {data: 'sl', name: 'sl'},
                {data: 'type', name: 'type'},
                {data: 'name', name: 'name'},
